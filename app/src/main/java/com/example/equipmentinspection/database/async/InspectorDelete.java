...
public class InspectorDelete extends AsyncTask<InspectorEntity, Void, Void> {

        private AppDatabase database;
    private OnAsyncEventListener callback;
    private Exception exception;

    public InspectorDelete(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(InspectorEntity... params) {
        try {
            for (InspectorEntity inspector : params)
                database.inspectorDao().delete(inspector);
        } catch (Exception e) {
            exception = e;
        }
        return null;
