...
public class EquipmentDelete extends AsyncTask<EquipmentEntity, Void, Void> {

    private AppDatabase database;
    private OnAsyncEventListener callback;
    private Exception exception;

    public EquipmentDelete(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(EquipmentEntity... params) {
        try {
            for (EquipmentEntity equipment : params)
                database.equipmentDao().delete(equipment);
        } catch (Exception e) {
            exception = e;
        }
        return null;
