...
public class InspectorCreate extends AsyncTask<InspectorEntity, Void, Void> {

        private AppDatabase database;
    private OnAsyncEventListener callback;
    private Exception exception;

    public InspectorCreate(Context context, OnAsyncEventListener callback) {
    ...
    protected Void doInBackground(InspectorEntity... params) {
        try {
            for (InspectorEntity inspector : params)
                database.inspectorDao().insert(inspector);
        } catch (Exception e) {
            exception = e;
        }
        return null;
